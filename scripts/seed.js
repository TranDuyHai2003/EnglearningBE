...
const randomSubset = (array, min = 1, max = array.length) => {
  ...
  const count =
    safeMin === safeMax
      ? safeMin
      : faker.number.int({ min: safeMin, max: safeMax });
  ...
  for (const instructor of instructors) {
    ...
    const [profile] = await InstructorProfile.findOrCreate({
      ...
      defaults: {
        ...
        approved_at:
          status === "approved" ? faker.date.recent({ days: 45 }) : null,
        rejection_reason:
          status === "rejected" ? "Need more teaching experience" : null,
      },
    });
    ...
    if (profile.approval_status !== status) {
      await profile.update({
        ...
        rejection_reason:
          status === "rejected" ? "Missing verification documents" : null,
      });
    }
  }
};
  ...
  const seeds = [
    {
      ...
      description: "Improve your pronunciation and accent.",
    },
    {
      name: "Everyday Speaking",
      description: "Speaking practice for everyday conversations.",
    },
    {
    ...
    },
    {
      ...
      description: "Prepare for IELTS tests with practical lessons.",
    },
    {
    ...
    {
      name: "Travel English",
      description: "Essential English for travellers.",
    },
  ];
...
const createCourseContent = async (course, tags) => {
  ...
  for (let i = 1; i <= sectionCount; i += 1) {
    ...
    for (let j = 1; j <= lessonCount; j += 1) {
      ...
      //   video_url:
      //     lessonType === "video"
      //       ? `https://videos.englearning.test/${slugify(
      //           course.title
      //         )}-${i}-${j}.mp4`
      //       : null,
      //   video_duration:
      //     lessonType === "video" ? faker.number.int({ min: 180, max: 900 }) : 0,
      ...
      const lesson = await Lesson.create({
        ...
        video_url:
          lessonType === "video"
            ? `https://www.youtube.com/watch?v=${randomItem(youtubeVideoIds)}`
            : null,
        video_duration:
          lessonType === "video" ? faker.number.int({ min: 180, max: 900 }) : 0,
        content: lessonType !== "video" ? faker.lorem.paragraphs(2) : null,
      ...
      if (faker.datatype.boolean()) {
        ...
        for (let k = 0; k < resourceCount; k += 1) {
          await LessonResource.create({
            ...
            file_url: `https://cdn.englearning.test/resources/${
              lesson.lesson_id
            }-${k + 1}.pdf`,
  ...
  for (const instructor of instructors) {
    ...
    for (let i = 0; i < courseCount; i += 1) {
      ...
      const discount = faker.datatype.boolean()
        ? Math.max(
            price -
              faker.number.float({ min: 3, max: price - 1, multipleOf: 0.5 }),
            0
          )
        : null;
      ...
      const course = await Course.create({
        ...
        title,
        slug: `${slugify(title)}-${faker.string
        ...
        reviewed_at:
          approval === "approved" ? faker.date.recent({ days: 60 }) : null,
        rejection_reason:
          approval === "rejected" ? "Content quality below standards" : null,
        total_students: 0,
        average_rating: 0,
        published_at:
          status === "published" ? faker.date.recent({ days: 90 }) : null,
      });
    ...
    for (const course of courseSubset) {
      ...
      const [enrollment] = await Enrollment.findOrCreate({
        ...
        defaults: {
          ...
          status,
          completed_at:
            completion === 100 ? faker.date.recent({ days: 40 }) : null,
...
const createLessonProgress = async (enrollments, contentByCourse) => {
  ...
  for (const enrollment of enrollments) {
    ...
    if (!content) continue;
    const subset = randomSubset(
      content.lessons,
      1,
      Math.min(8, content.lessons.length)
    );
...
const createTransactions = async (enrollmentsByStudent, courses) => {
  ...
  for (const [
    studentId,
    studentEnrollments,
  ] of enrollmentsByStudent.entries()) {
    ...
    for (let t = 0; t < transactionCount; t += 1) {
      ...
      const courseRecords = chosenEnrollments
        .map((enrollment) =>
          courses.find((course) => course.course_id === enrollment.course_id)
        )
        .filter(Boolean);
...
const createReviews = async (enrollments) => {
  ...
  const reviews = [];
  for (const enrollment of enrollments) {
    if (enrollment.completion_percentage < 40 || !faker.datatype.boolean())
      continue;
  ...
  for (const enrollment of enrollments) {
    ...
    for (const lesson of lessonsForCourse) {
      ...
      if (faker.datatype.boolean()) {
        ...
        const replyUserId =
          faker.datatype.boolean() && instructorId
            ? instructorId
...
const issueCertificates = async (enrollments) => {
  ...
  for (const enrollment of enrollments) {
    ...
    if (!existing) {
      const code = `CERT-${faker.string.alphanumeric(8).toUpperCase()}`;
      await Certificate.create({
        ...
        expiry_date: null, // Lifetime validity
        verify_url: `https://englearning.test/verify/${code}`,
      });
      count++;
    }
  }
  log(`Issued ${count} new certificates.`);
};
    ...
    for (let i = 0; i < attemptCount; i += 1) {
      ...
      for (const questionEntry of quizEntry.questions) {
        ...
        await StudentAnswer.create({
          ...
          points_earned: chosenOption.is_correct
            ? Number(questionEntry.question.points || 1)
            : 0,
        });
...
const main = async () => {
  try {
    ...
    const { enrollments, enrollmentsByStudent } = await createEnrollments(
      students,
      courses
    );
    ...
    await issueCertificates(enrollments);
    await createDiscussionsAndMessages(
      enrollments,
      contentByCourse,
      instructors,
      students,
