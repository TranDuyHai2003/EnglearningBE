...
const up = async () => {
  ...
  const safeAddIndex = async (tableName, fields, options) => {
    try {
      ...
      console.log(`Index ${options.name} added to ${tableName}`);
    } catch (error) {
  ...
  // 1. flashcard_decks
  if (!(await tableExists("flashcard_decks"))) {
    ...
    console.log("Created table flashcard_decks");
  } else {
    console.log("Table flashcard_decks already exists");
  }

  await safeAddIndex("flashcard_decks", ["owner_user_id"], {
    name: "idx_flashcard_decks_owner",
  });
  await safeAddIndex("flashcard_decks", ["visibility"], {
    name: "idx_flashcard_decks_visibility",
  });

  // 2. flashcards
  if (!(await tableExists("flashcards"))) {
    ...
    console.log("Table flashcards already exists");
  }

  await safeAddIndex("flashcards", ["deck_id"], {
    name: "idx_flashcards_deck",
  });
  await safeAddIndex("flashcards", ["owner_user_id"], {
    name: "idx_flashcards_owner",
  });

  // 3. flashcard_user_state
  if (!(await tableExists("flashcard_user_state"))) {
    ...
    console.log("Created table flashcard_user_state");
  } else {
    console.log("Table flashcard_user_state already exists");
  }
  ...
  // 4. flashcard_reviews
  if (!(await tableExists("flashcard_reviews"))) {
    ...
    console.log("Created table flashcard_reviews");
  } else {
    console.log("Table flashcard_reviews already exists");
  }
